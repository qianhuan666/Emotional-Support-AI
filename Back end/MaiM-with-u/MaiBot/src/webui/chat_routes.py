...
# 简单内存限流：每个用户在时间窗口内的请求次数上限
RATE_LIMIT_WINDOW_SECONDS = 60.0
RATE_LIMIT_MAX_REQUESTS = 20
_rate_limit_bucket: Dict[str, deque] = {}
_rate_limit_lock = threading.Lock()

# 关键词拦截（基础版，可后续接入外部审核服务）
BLOCKED_KEYWORDS = [
    "买粉",
    "代开发票",
    "博彩",
...
def _check_and_consume_rate_limit(user_id: str) -> float:
    """检查并消耗一次配额。
    ...
    with _rate_limit_lock:
        bucket = _rate_limit_bucket.get(user_id)
        if bucket is None:
            bucket = deque()
        ...
        if len(bucket) >= RATE_LIMIT_MAX_REQUESTS:
            wait_for = RATE_LIMIT_WINDOW_SECONDS - (now - bucket[0])
            return max(0.1, wait_for)

        bucket.append(now)
        return 0.0


def _match_blocked_keyword(content: str) -> Optional[str]:
    """命中拦截词返回关键词，否则返回 None。"""
...
def create_message_data(
    ...
    if virtual_config and virtual_config.enabled:
        ...
        group_id = conversation_id or WEBUI_CHAT_GROUP_ID
        group_name = f"WebUI会话 {group_id[-8:]}" if conversation_id else "WebUI本地聊天室"
...
def _is_bot_message(user_id: str, group_id: str) -> bool:
    """判断某条消息是否来自机器人。"""
    if group_id.startswith(VIRTUAL_GROUP_ID_PREFIX):
        return user_id == str(global_config.bot.qq_account)
    return not user_id.startswith("webui_") and not user_id.startswith(WEBUI_USER_ID_PREFIX)


def _get_latest_bot_reply_after(group_id: str, start_time: float) -> Optional[Dict[str, Any]]:
    """获取指定时间之后最新的一条机器人回复。"""
    messages = (
        Messages.select()
        .where((Messages.chat_info_group_id == group_id) & (Messages.time >= start_time))
        .order_by(Messages.time.desc())
        .limit(20)
    )
    for msg in messages:
        user_id = msg.user_id or ""
        if _is_bot_message(user_id, group_id):
            content = msg.processed_plain_text or msg.display_message or ""
            return {"message_id": msg.message_id, "content": content}
    return None
...
async def sync_chat(request: SyncChatRequest, _auth: bool = Depends(require_auth)):
    """普通同步回复接口：发送消息并等待机器人回复。"""
    content = (request.content or "").strip()
    if not content:
        raise HTTPException(status_code=400, detail="content 不能为空")
    hit_keyword = _match_blocked_keyword(content)
    if hit_keyword:
        logger.warning(f"内容审核拦截(sync): keyword={hit_keyword}")
        raise HTTPException(status_code=403, detail=f"消息包含受限内容（命中关键词：{hit_keyword}）")

    timeout_seconds = max(3.0, min(float(request.timeout_seconds or 20.0), 60.0))
    retry_count = max(0, min(int(request.retry_count or 0), 3))
    user_id = request.user_id or f"{WEBUI_USER_ID_PREFIX}{uuid.uuid4().hex[:16]}"
    if not user_id.startswith(WEBUI_USER_ID_PREFIX):
        user_id = f"{WEBUI_USER_ID_PREFIX}{user_id}"
    wait_seconds = _check_and_consume_rate_limit(user_id)
    if wait_seconds > 0:
        raise HTTPException(
            status_code=429,
            detail=f"请求过于频繁，请在 {wait_seconds:.1f} 秒后重试",
    ...
    last_error = None
    hard_error = None
    group_id = request.conversation_id or WEBUI_CHAT_GROUP_ID
    ...
    for attempt in range(retry_count + 1):
        ...
        message_data = create_message_data(
            ...
            message_id=user_message_id if attempt == 0 else str(uuid.uuid4()),
            is_at_bot=True,
            virtual_config=None,
        ...
        try:
            ...
            last_error = e
            logger.warning(f"同步回复第 {attempt + 1} 次尝试失败: {e}")
        ...
        while time.time() < deadline:
            bot_reply = _get_latest_bot_reply_after(group_id, start_time)
            if bot_reply and bot_reply.get("content"):
                return SyncChatResponse(
                    success=True,
                    user_message_id=user_message_id,
                    reply=bot_reply["content"],
                    reply_message_id=bot_reply.get("message_id"),
                    timeout=False,
                )
            await asyncio.sleep(0.5)

        last_error = TimeoutError("等待回复超时")
        logger.warning(f"同步回复第 {attempt + 1} 次尝试超时")
        if attempt < retry_count:
            await asyncio.sleep(_retry_delay_seconds(attempt))

    if hard_error is not None:
        raise HTTPException(status_code=500, detail=f"处理消息失败（不可重试）: {hard_error}")

    if isinstance(last_error, TimeoutError):
        return SyncChatResponse(
            success=False,
            user_message_id=user_message_id,
            reply=f"等待回复超时，已重试 {retry_count} 次。",
            reply_message_id=None,
            timeout=True,
        )
    raise HTTPException(status_code=500, detail=f"处理消息失败（已重试 {retry_count} 次）: {last_error}")
...
def _retry_delay_seconds(attempt_index: int) -> float:
    """指数退避：0.8, 1.6, 3.2 ... (上限 8s)。"""
    return min(0.8 * (2**attempt_index), 8.0)


def _is_retryable_error(err: Exception) -> bool:
    """仅对临时失败重试，参数/逻辑错误不重试。"""
    if isinstance(err, (TimeoutError, asyncio.TimeoutError, ConnectionError)):
        return True
    msg = str(err).lower()
    retryable_keywords = [
        "timeout",
...
@router.post("/sse")
async def sse_chat(request: SyncChatRequest, _auth: bool = Depends(require_auth)):
    """流式回复接口（SSE）。"""
    content = (request.content or "").strip()
    if not content:
        raise HTTPException(status_code=400, detail="content 不能为空")
    hit_keyword = _match_blocked_keyword(content)
    if hit_keyword:
        logger.warning(f"内容审核拦截(sse): keyword={hit_keyword}")
        raise HTTPException(status_code=403, detail=f"消息包含受限内容（命中关键词：{hit_keyword}）")

    timeout_seconds = max(3.0, min(float(request.timeout_seconds or 20.0), 90.0))
    retry_count = max(0, min(int(request.retry_count or 0), 3))
    user_id = request.user_id or f"{WEBUI_USER_ID_PREFIX}{uuid.uuid4().hex[:16]}"
    if not user_id.startswith(WEBUI_USER_ID_PREFIX):
        user_id = f"{WEBUI_USER_ID_PREFIX}{user_id}"
    ...
    async def event_generator():
        # 通知前端请求已接收
        yield _sse_event("start", {"success": True, "user_message_id": user_message_id})
        last_error = None
        hard_error = None
        for attempt in range(retry_count + 1):
            local_start_time = time.time()
            local_message_data = create_message_data(
                content=content,
                user_id=user_id,
                user_name=request.user_name,
                message_id=user_message_id if attempt == 0 else str(uuid.uuid4()),
            ...
            process_task = asyncio.create_task(chat_bot.message_process(local_message_data))
            deadline = time.time() + timeout_seconds
            try:
                bot_reply = None
                while time.time() < deadline:
                    bot_reply = _get_latest_bot_reply_after(request.conversation_id or WEBUI_CHAT_GROUP_ID, local_start_time)
                    if bot_reply and bot_reply.get("content"):
                        break
                    yield _sse_event("heartbeat", {"ts": time.time(), "attempt": attempt + 1})
                ...
                if bot_reply and bot_reply.get("content"):
                    full_text: str = bot_reply["content"]
                    chunk_size = 18
                    cursor = 0
                    while cursor < len(full_text):
                        piece = full_text[cursor : cursor + chunk_size]
                        cursor += chunk_size
                        yield _sse_event("chunk", {"content": piece})
                ...
                last_error = TimeoutError("等待回复超时")
                if attempt < retry_count:
                    yield _sse_event("retry", {"attempt": attempt + 1, "reason": "timeout"})
                    await asyncio.sleep(_retry_delay_seconds(attempt))
            except Exception as e:
                last_error = e
                logger.warning(f"SSE 第 {attempt + 1} 次尝试失败: {e}")
                if not _is_retryable_error(e):
                    hard_error = e
                    break
                if attempt < retry_count:
                    yield _sse_event("retry", {"attempt": attempt + 1, "reason": str(e)})
                    await asyncio.sleep(_retry_delay_seconds(attempt))
            finally:
                if not process_task.done():
                    process_task.cancel()
        ...
        if hard_error is not None:
            yield _sse_event("error", {"success": False, "message": f"处理消息失败（不可重试）: {hard_error}"})
            return

        if isinstance(last_error, TimeoutError):
            yield _sse_event("error", {"success": False, "timeout": True, "message": f"等待回复超时，已重试 {retry_count} 次。"})
        else:
            yield _sse_event("error", {"success": False, "message": f"处理消息失败（已重试 {retry_count} 次）: {last_error}"})
    ...
    try:
        ...
        while True:
            ...
            if data.get("type") == "message":
                ...
                hit_keyword = _match_blocked_keyword(content)
                if hit_keyword:
                    logger.warning(f"内容审核拦截(ws): keyword={hit_keyword}")
                    await chat_manager.send_message(
                        session_id,
                        {
                            "type": "system",
                            "content": f"消息未发送：包含受限内容（命中关键词：{hit_keyword}）",
                            "timestamp": time.time(),
                        },
                    )
                    continue
