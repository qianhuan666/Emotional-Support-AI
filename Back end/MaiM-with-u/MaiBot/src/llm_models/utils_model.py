...
class LLMRequest:
    ...
    async def _execute_request(
        ...
        for _ in range(max_attempts):
            ...
            try:
                ...
                llm_usage_recorder.record_failure_to_database(
                    model_info=model_info,
                    user_id="system",
                    request_type=self.request_type,
                    endpoint=self._endpoint_for_request_type(request_type),
                    error_message=str(last_exception),
                    time_cost=time.time() - request_start_time,
                )
                total_tokens, penalty, usage_penalty = self.model_usage[model_info.name]
    ...
    @staticmethod
    def _endpoint_for_request_type(request_type: RequestType) -> str:
